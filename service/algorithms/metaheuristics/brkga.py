...
class BRKGA:
    ...
    def solve(
        ...
        # Inicialização e avaliação da população inicial
        pop = [[random.random() for _ in range(n)] for _ in range(self.pop_size)]
        pop_fitness = [(k, eval_keys(k)) for k in pop]
        pop_fitness.sort(key=lambda x: (x[1].total_penalty, x[1].total_route_time))
        
        # A população começa ordenada
        pop = [x[0] for x in pop_fitness]
        best_keys = pop[0]
        best_eval = pop_fitness[0][1]
        no_improve = 0
        ...
        for _ in range(self.max_gens):
            # A população 'pop' já está ordenada (da inicialização ou iteração anterior)
            elites = pop[:elite_size]
            non_elites = pop[elite_size:]

            next_pop = []
            next_pop.extend(elites)
            
            # Crossover: Criar nova geração
            while len(next_pop) < self.pop_size - mutant_size:
            ...
            # Mutantes: Adicionar diversidade
            ...
            # Avaliar e ordenar a NOVA população (Filhos)
            pop_fitness = [(k, eval_keys(k)) for k in pop]
            pop_fitness.sort(key=lambda x: (x[1].total_penalty, x[1].total_route_time))
            
            pop = [x[0] for x in pop_fitness]

            # Verificar melhoria na NOVA população (igual ao original)
            current_best_keys = pop[0]
            current_best_eval = pop_fitness[0][1]
